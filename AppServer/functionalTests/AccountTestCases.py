...
class AccountTest(unittest.TestCase):
	...
	def test_3(self):
		payload = {'username': "'"+ self._username_long +"'",'password':'pass'}
		r = requests.post(self.__api_base_url + self._url_usuario + "signup", json=payload)
	...
	def test_9(self):
		payload = {'username': "'lalal'",'password':'pass'}
		r = requests.post(self.__api_base_url + self._url_usuario + "login", json=payload)
	...
	def test_10(self):
		payload = {'username': "'"+ self._username +"'",'password':'passdifferent'}
		r = requests.post(self.__api_base_url + self._url_usuario + "login", json=payload)
		self.assertEqual(r.status_code, 200)
		data = json.loads(r.text)
	...
	def test_14(self):
		payload = {'username': "'" + self._usernamelike + "'",'password':'pass'}
		r = requests.post(self.__api_base_url + self._url_usuario + "signup", json=payload)
		self.assertEqual(r.status_code, 200)
		data = json.loads(r.text)
		self.assertEqual(data["data"]["message"], "Successful signup")
